...
export default function OTPVerification() {
  ...
  const handleVerify = async (e) => {
    e.preventDefault();
    if (otp.length !== 6) {
      toast.warning("⚠️ Vui lòng nhập đủ 6 số OTP"); 
      return;
    }
      ...
      <div className="max-w-2xl mx-auto w-full flex-1 flex flex-col">
        ...
        <nav className="bg-[#1e2d40] px-6 py-4 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-3">
            {/* Ẩn nút back vì đang trong luồng OTP không nên quay lại, chỉ hiện Text */}
            ...
            <div>
              ...
              <p className="text-white/40 text-[10px] uppercase tracking-widest">Xác thực bước 2</p>
            </div>
          </div>
          {/* Accent line nhỏ trên góc để nhấn mạnh đang xử lý */}
          <div className="w-2 h-2 rounded-full bg-[#2563eb] animate-pulse" />
        </nav>

        {/* ── Vùng nội dung trung tâm ── */}
        <div className="p-6 flex-1 flex items-center justify-center">
          <div className="w-full max-w-md bg-white p-8 rounded-2xl shadow-sm border border-gray-100 text-center relative overflow-hidden">
            
            {/* Thanh loading accent khi đang xử lý API */}
            {isLoading && <div className="absolute top-0 left-0 w-full h-1 bg-[#2563eb] animate-pulse"></div>}

            {/* Icon bảo mật */}
            <div className="w-12 h-12 bg-[#f4f6f9] border border-gray-100 rounded-xl flex items-center justify-center mx-auto mb-5 relative">
              <svg className="w-6 h-6 text-[#1e2d40] z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path>
              </svg>
            </div>

            <h2 className="text-sm font-semibold text-gray-800 mb-1.5 uppercase tracking-wide">Xác nhận giao dịch</h2>
            <p className="text-[11px] font-medium text-gray-400 mb-8 px-4">
              Mã OTP gồm 6 chữ số đã được gửi đến thiết bị và Email đăng ký của bạn.
            </p>
            ...
            <form onSubmit={handleVerify}>
              <input 
                ...
                className={`w-full text-center text-3xl tracking-[0.4em] font-bold py-4 rounded-xl border focus:outline-none transition-all mb-6
                  ${timeLeft === 0 
            ...
            <div className="mt-6 pt-6 border-t border-gray-50">
              <div className="flex items-center justify-center gap-2 text-[10px] font-semibold uppercase tracking-widest">
                <span className="text-gray-400">Thời gian hiệu lực:</span>
                <span className={`font-mono text-xs tracking-wider transition-colors ${timeLeft <= 30 ? 'text-[#b91c1c] animate-pulse' : 'text-[#1e2d40]'}`}>
                  {formatTime(timeLeft)}
                </span>
              </div>
              ...
              {timeLeft === 0 && (
                <button 
                    onClick={() => navigate('/dashboard')}
                    className="mt-5 w-full py-2.5 border border-gray-200 text-gray-500 text-[10px] font-semibold uppercase tracking-widest rounded-xl hover:border-[#1e2d40] hover:text-[#1e2d40] transition-colors active:scale-[0.98]"
                >
                  Về trang chủ & Tạo lệnh mới
                </button>
              )}
            </div>
