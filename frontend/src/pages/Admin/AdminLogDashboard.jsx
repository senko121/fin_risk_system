...
export default function AdminLogDashboard() {
  ...
  // Khi đổi Tab -> Reset mọi thứ về 0 và gọi lại
  useEffect(() => {
    setLogs([]); // Xóa dữ liệu cũ của tab trước
    setPage(0);
    setHasMore(true);
    fetchLogs(0, activeTab);
  }, [activeTab]);

  const fetchLogs = async (pageNumber, currentTab) => {
    if (pageNumber === 0) setLoading(true);
    else setIsLoadingMore(true);

    try {
      const endpoint = currentTab === 'audit' ? '/admin/logs/audit' : '/admin/logs/config';
      // Gọi API kèm tham số page
      const res = await axiosClient.get(`${endpoint}?page=${pageNumber}&size=10`);
      
      // Lấy data từ cục content (Do Spring Boot trả về dạng Page)
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                   ...
                   <tr><td colSpan="4" className="text-center py-10 font-bold text-slate-400 italic">Không có dữ liệu.</td></tr>
                ) : logs.map((log) => (
                  <tr key={`log_${activeTab}_${log.id}`} className="hover:bg-blue-50/30 transition-colors">
                    ...
                    <td className="px-6 py-5 w-56">
                      <div className="flex items-center space-x-3">
                        <div className="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center font-bold text-slate-600 text-xs uppercase">
                          {(log.username || log.adminUsername)?.charAt(0) || '?'}
                        </div>
                        <span className="font-bold text-slate-700">{log.username || log.adminUsername}</span>
                      </div>
                    </td>
          ...
          {!loading && logs.length > 0 && (
            <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-center">
              {hasMore ? (
                ...
                >
                  {isLoadingMore ? 'Đang tải thêm...' : '⬇️ Tải thêm nhật ký cũ hơn'}
                </button>
              ) : (
