  ...
  return (
    <div className="min-h-screen bg-[#f4f6f9]">
      <div className="max-w-2xl mx-auto min-h-screen">
        ...
        {/* ── Topbar: navy shell ── */}
        <nav className="bg-[#1e2d40] px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-white tracking-wide">FINRISK</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => navigate('/account')}
              className="flex items-center gap-2 px-2 py-1.5 hover:bg-white/10 rounded-lg transition-colors group"
            >
              <div className="w-8 h-8 bg-white/10 text-white/80 rounded-full flex items-center justify-center font-semibold text-sm border border-white/15 group-hover:border-white/30 transition-all">
                {user.fullName?.charAt(0)}
              </div>
            ...
            <button
              ...
              className="p-2 text-white/40 hover:text-red-300 hover:bg-red-500/10 rounded-lg transition-all"
              title="Đăng xuất"
            >
        ...
        <div className="p-6 space-y-5">
          ...
          {/* ── Balance card: navy shell, nhất quán với topbar ── */}
          <div className="bg-[#1e2d40] rounded-2xl p-6 text-white relative overflow-hidden">

            {/* Accent line xanh — điểm nhấn duy nhất có màu */}
            ...
            <div className="flex justify-between items-start mb-1">
              ...
              <button
                onClick={() => setIsQRModalOpen(true)}
                className="bg-white/10 hover:bg-white/20 border border-white/15 p-2 rounded-lg transition-all"
                title="Hiển thị mã QR nhận tiền"
              >
            ...
            <div className="flex justify-between items-end pt-4 border-t border-white/10">
              <div>
                ...
                <p className="font-mono text-sm tracking-widest text-white/70">
                  {user.accountNumber
                    ? String(user.accountNumber).replace(/(.{4})/g, '$1 ').trim()
                    : 'Đang cập nhật'}
                </p>
              </div>
              {/* Amber badge = premium tier — có nghĩa, không trang trí */}
              <div className="bg-amber-500/15 border border-amber-400/20 text-amber-300 px-3 py-1 rounded-lg text-[10px] font-semibold uppercase tracking-widest">
                Global Class
              </div>
            </div>

            {/* Subtle shape — không dùng blur */}
            <div className="absolute -bottom-8 -right-8 w-32 h-32 bg-white/[0.03] rounded-full pointer-events-none" />
          </div>
            ...
            <div className="grid grid-cols-4 gap-3">
              {navItems.map((item) => (
                <button
                  ...
                  className="flex flex-col items-center gap-2.5 py-4 px-2 bg-white rounded-xl border border-gray-100 hover:border-[#1e2d40] hover:bg-[#1e2d40] transition-all group"
                >
                  <div className="w-10 h-10 bg-[#f4f6f9] text-gray-500 rounded-xl flex items-center justify-center group-hover:bg-white/10 group-hover:text-white transition-all">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      {item.icon}
                    </svg>
                  </div>
                  <p className="text-[11px] font-semibold text-gray-600 group-hover:text-white/80 transition-colors text-center leading-tight">
                    {item.label}
                  </p>
                </button>
              ))}
          ...
          {/* ── Coming soon: full width, dashed ── */}
          <button
            disabled
            className="w-full py-4 bg-white rounded-xl border border-dashed border-gray-200 opacity-40 cursor-not-allowed flex items-center justify-center gap-2"
          >
