...
class AdminUserServiceTest {
    ...
    class GetAllUsersTests {
        ...
        @Test
        @DisplayName("Trả về Page rỗng khi không có user")
        void shouldReturnEmptyListWhenNoUsers() {
            Page<User> emptyPage = new PageImpl<>(Collections.emptyList());
            when(userRepository.searchUsers(anyString(), any(Pageable.class))).thenReturn(emptyPage);

            assertThat(adminUserService.getUsers("", 0, 10).getContent()).isEmpty();
        }
        ...
        void shouldMapFieldsCorrectly() {
            ...
            AdminUserDTO dto = adminUserService.getUsers("", 0, 10).getContent().get(0);
        ...
        @Test
        @DisplayName("hasFaceData = true khi user có faceEmbeddings")
        void shouldSetHasFaceDataTrueWhenFaceImageExists() {
            User u = buildUser(1L, "faceuser", "ACTIVE");
            u.setFaceEmbeddings("[[0.1,0.2,0.3]]");
            Page<User> userPage = new PageImpl<>(List.of(u));
            when(userRepository.searchUsers(anyString(), any(Pageable.class))).thenReturn(userPage);
        ...
        @Test
        @DisplayName("hasFaceData = false khi faceEmbeddings là null")
        void shouldSetHasFaceDataFalseWhenFaceImageNull() {
            User u = buildUser(1L, "noface", "ACTIVE");
            // faceEmbeddings = null by default
            Page<User> userPage = new PageImpl<>(List.of(u));
            when(userRepository.searchUsers(anyString(), any(Pageable.class))).thenReturn(userPage);

            assertThat(adminUserService.getUsers("", 0, 10).getContent().get(0).isHasFaceData()).isFalse();
        }

        @Test
        @DisplayName("hasFaceData = false khi faceEmbeddings là chuỗi trắng")
        ...
        void shouldSetSuspiciousTrueWhenAdminFlagged() {
            ...
            assertThat(adminUserService.getUsers("", 0, 10).getContent().get(0).isSuspiciousSession()).isTrue();
        }
    ...
    class ResetFaceBiometricTests {
        ...
        @Test
        @DisplayName("Xóa khuôn mặt thành công: faceEmbeddings phải là null sau khi save")
        void shouldClearFaceImageSuccessfully() {
            ...
            ArgumentCaptor<User> captor = ArgumentCaptor.forClass(User.class);
            verify(userRepository).save(captor.capture());
            assertThat(captor.getValue().getFaceEmbeddings()).isNull();
        }
        ...
        @Test
        @DisplayName("Ném RuntimeException khi faceEmbeddings là chuỗi trắng")
        void shouldThrowWhenFaceImageIsBlank() {
            User user = buildUser(1L, "alice", "ACTIVE");
            user.setFaceEmbeddings("   ");
            when(userRepository.findById(1L)).thenReturn(Optional.of(user));
        ...
        @Test
        @DisplayName("Phải ghi ConfigLog với oldJson chứa nhãn [ĐÃ_BỊ_HỦY] và newJson là null")
        void shouldWriteConfigLogWithSafeLabel() {
            ...
            assertThat(log.getOldValue()).contains("ĐÃ_BỊ_HỦY");
    ...
    // ══════════════════════════════════════════════════════════════
    @Nested
    @DisplayName("convertToDTO() – logic mapping (via getUsers)")
    class ConvertToDTOTests {
        ...
        void shouldBeFalseWhenBothFlagsAreFalse() {
            ...
            assertThat(adminUserService.getUsers("", 0, 10).getContent().get(0).isSuspiciousSession()).isFalse();
        }
        ...
        void shouldMapCreatedAt() {
            ...
            assertThat(adminUserService.getUsers("", 0, 10).getContent().get(0).getCreatedAt()).isEqualTo(expected);
        }
