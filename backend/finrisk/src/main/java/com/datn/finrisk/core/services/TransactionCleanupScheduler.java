...
public class TransactionCleanupScheduler {
    ...
    public void expireStaleUnderReviewTransactions() {
        ...
        System.out.println("🔍 [SCHEDULER] Tìm thấy " + staleIds.size()
            + " giao dịch UNDER_REVIEW quá 24h chờ kiểm duyệt.");
        ...
        System.out.println("⏰ [SCHEDULER] Đã đánh dấu EXPIRED: " + expired
            + "/" + staleIds.size() + " giao dịch UNDER_REVIEW.");
    }
    ...
    public void cleanupStalledTransactions() {
        ...
        System.out.println("🧹 [SCHEDULER] Tìm thấy " + stalledIds.size() + " giao dịch bị treo...");
        ...
        for (Long txId : stalledIds) {
            ...
            System.out.println("⏰ [SCHEDULER] Đã hủy giao dịch treo: #" + txId);
        }

        System.out.println("⏰ [SCHEDULER] Đã đánh dấu EXPIRED: " + expired
            + "/" + stalledIds.size() + " giao dịch bị treo.");
    }
}